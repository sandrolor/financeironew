                    ...
                    <div class="card-body">
                        <form action="{{ route('movimento.index') }}" method="GET" class="form-inline">
                            <div class="form-group mx-sm-3 mb-2">
                                <input type="text" class="form-control-sm col-2 m-1" name="pesquisar" id="pesquisar" placeholder="Digite o nome" />
                                ...
                                <a href="{{ route('cadastrar.movimento') }}" type="button" class="btn btn-success float-end">Novo Movimento</a>
                            </div>
                        </form>
                            ...
                            <table class="table table-striped table-sm">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col">Valor | {{ 'R$' . ' ' . number_format($totalMov, 2, ',', '.') }}</th>
                                ...
                                <tbody>
                                    @foreach ($findMovimento as $movimento)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('atualizar.movimento', $movimento->id) }}" class="btn btn-light btn-sm">Editar</a>
                                                <form method="POST" action="{{ route('movimento.delete' , $movimento->id) }}" accept-charset="UTF-8" style="display:inline">
                                                    {{ method_field('DELETE') }}
                                                    {{ csrf_field() }}
